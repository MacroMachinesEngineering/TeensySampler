...
void setup(){
  ...
  waveform3.arbitraryWaveform(buffer3, 344);
  waveform4.begin(0.8, tableFreq, WAVEFORM_ARBITRARY);
  waveform4.arbitraryWaveform(buffer4, 344);
  sine1.amplitude(0.8);
  sine2.amplitude(0.8);
  sine3.amplitude(0.8);
  sine4.amplitude(0.8);
...
void loop(){
  ...
  if(queue2.available() >= 4){ // input buffer
    memcpy(buffer1, queue2.readBuffer(), 256);
    queue2.freeBuffer();
    memcpy(buffer2, queue2.readBuffer(), 256);
    queue2.freeBuffer();
    memcpy(buffer3, queue2.readBuffer(), 256);
    queue2.freeBuffer();
    memcpy(buffer4, queue2.readBuffer(), 256);
    queue2.freeBuffer();
    ...
    sine1.phase(0);
    sine1.frequency(tableFreq/4);
    
    waveform2.phase(90);
    waveform2.frequency(tableFreq);
    sine2.phase(90);
    sine2.frequency(tableFreq/4);
    
    waveform3.phase(180);
    waveform3.frequency(tableFreq);
    sine3.phase(180);
    sine3.frequency(tableFreq/4);
    
    waveform4.phase(270);
    waveform4.frequency(tableFreq);
    sine4.phase(270);
    sine4.frequency(tableFreq/4);
    AudioInterrupts();
